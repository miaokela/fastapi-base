...
# 注册认证路由 - 使用 router 的 add_api_route 方法
# 第一个参数是路径，第二个参数是处理函数
router.add_api_route("/auth/register", AuthViewSet.register, methods=["POST"], summary="用户注册")
router.add_api_route("/auth/login", AuthViewSet.login, methods=["POST"], summary="用户登录")  
router.add_api_route("/auth/me", AuthViewSet.get_me, methods=["GET"], summary="获取当前用户信息")
