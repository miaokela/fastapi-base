...
class ComplexQueryBuilder:
    ...
    def __init__(self, model: Optional[Model] = None, sql_dir: Optional[str] = None):
        self.model = model
        self.table_name = model._meta.db_table if model else None
        
        # 设置SQL模板目录
        if sql_dir is None:
            sql_dir = str(Path(__file__).parent.parent.parent / "sql")
        
        # 使用 builder() 函数创建 PyQueryBuilder 实例
        self.query_builder = builder()
        self.query_builder.sql_path = sql_dir
    ...
    def build_with_template(self, template_name: str, **kwargs) -> str:
        """使用模板构建复杂SQL查询"""
        return self.query_builder.build(template_name, **kwargs)
    ...
    async def execute_template(self, template_name: str, **kwargs) -> List[Dict[str, Any]]:
        """使用模板执行复杂查询"""
        sql = self.build_with_template(template_name, **kwargs)
        connection = Tortoise.get_connection("default")
        result = await connection.execute_query(sql)
        return result[1]
