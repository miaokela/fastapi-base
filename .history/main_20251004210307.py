...
def register_routes():
    ...
    # 包含认证路由（已在 user_views.py 中通过 AuthViewSet.add_api_route 注册）
    app.include_router(user_router, tags=["认证"])
    
    # 使用 viewset_routes 自动注册 ViewSet 的所有 CRUD 路由
    # 用户管理路由 - 自动生成 GET/POST/PUT/PATCH/DELETE
    viewset_routes(app, UserViewSet, prefix="/api/v1/users", tags=["用户管理"])
    
    # 用户资料路由 - 自动生成 GET/POST/PUT/PATCH/DELETE
    viewset_routes(app, UserProfileViewSet, prefix="/api/v1/profiles", tags=["用户资料"])
    
    # 文章路由 - 自动生成 GET/POST/PUT/PATCH/DELETE
    viewset_routes(app, PostViewSet, prefix="/api/v1/posts", tags=["文章管理"])
